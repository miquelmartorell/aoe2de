...
for file_matches in files_matches:
    ...
    matches["match_id"] = matches["match_id"].astype(pd.Int64Dtype())
    matches["version"] = matches["version"].astype(pd.Int64Dtype())
    matches["num_players"] = matches["num_players"].astype(pd.Int64Dtype())
    matches["started"] = matches["started"].astype(pd.Int64Dtype())
    matches["finished"] = matches["finished"].astype(pd.Int64Dtype())
    matches["duration"] = matches["duration"].astype(pd.Int64Dtype())
    matches["duration_minutes"] = matches["duration_minutes"].astype(pd.Int64Dtype())
    ...
    # Conversión de variables a int64.
    players["match_id"] = players["match_id"].astype(pd.Int64Dtype())
    players["profile_id"] = players["profile_id"].astype(pd.Int64Dtype())
    players["steam_id"] = players["steam_id"].astype(pd.Int64Dtype())
    players["slot"] = players["slot"].astype(pd.Int64Dtype())
    players["rating"] = players["rating"].astype(pd.Int64Dtype())
    players["rating_change"] = players["rating_change"].astype(pd.Int64Dtype())
    players["team"] = players["team"].astype(pd.Int64Dtype())
    players["won"] = players["won"].astype(int)
    ...
    # Eliminación de partidas duplicadas en players.
    players.drop_duplicates(subset=["match_id", "profile_id"], keep="first", inplace=True, ignore_index=True)
    ...
    matches_array = matches.iloc[:, 0].unique()
    players_array = players.iloc[:, 0].unique()
    if ((matches_array == players_array).all() == True):
        print("Integridad de los datos verificada correctamente.")
    else:
    ...
    matches.to_csv(path_or_buf= path_save_clean_matches + "/matches_clean_data_" + str(started_date) + ".csv", header=True, index=False, quoting=1, na_rep='NULL')
    path_save_clean_players = "/home/mms959tfm/data_aoe2_de/csv/clean_data/players_clean_data/"   
    players.to_csv(path_or_buf= path_save_clean_players + "/players_clean_data_" + str(started_date) + ".csv", header=True, index=False, quoting=1, na_rep='NULL')
